import { ExpenseWithAmount, BudgetAllocation } from './types.js';
...
export class BudgetCalculator {
   ...
   */
  calculateAllocation(totalBudget: number, expenses: ExpenseWithAmount[]): BudgetAllocation {
    // Subtract priority 1 & 2 expenses from budget
    const criticalExpenses = expenses.filter(e => e.priority <= 2);
    const criticalTotal = criticalExpenses.reduce((sum, e) => sum + e.amount, 0);
    ...
    const discretionaryTotal = discretionaryExpenses.reduce((sum, e) => sum + e.amount, 0);
